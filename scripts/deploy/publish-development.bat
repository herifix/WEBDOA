...
set "PWA_CONFIG=%ROOT%doa_donatur_pwa\.env.devpublish"
...
if not exist "%PWA_CONFIG%" (
  echo File config PWA development tidak ditemukan:
  echo %PWA_CONFIG%
  echo Jalankan dulu prepare-development-config.bat
  exit /b 1
)
...
if not errorlevel 1 (
  echo.
  echo PERINGATAN: .env.devpublish frontend masih berisi placeholder YOUR_*
  echo Silakan edit dulu file berikut:
  echo %CLIENT_CONFIG%
  exit /b 1
)
...
if not errorlevel 1 (
  echo.
  echo PERINGATAN: VITE_API_BASE_URL di frontend .env.devpublish masih kosong
  echo Silakan edit dulu file berikut:
  echo %CLIENT_CONFIG%
  exit /b 1
)
...
if not errorlevel 1 (
  echo.
  echo PERINGATAN: .env.devpublish PWA masih berisi placeholder YOUR_*
  echo Silakan edit dulu file berikut:
  echo %PWA_CONFIG%
  exit /b 1
)
...
if not errorlevel 1 (
  echo.
  echo PERINGATAN: VITE_API_BASE_URL di PWA .env.devpublish masih kosong
  echo Silakan edit dulu file berikut:
  echo %PWA_CONFIG%
  exit /b 1
)
...
if exist "%API_OUT%" (
  echo.
  echo [0/3] Stop API development lama jika masih berjalan...
...
echo.
echo [1/3] Build frontend UI (development)...
pushd "%CLIENT_DIR%"
...
echo.
echo [2/3] Build PWA (development)...
pushd "%PWA_DIR%"
call npm run build:devpublish
if errorlevel 1 (
  popd
...
echo.
echo [3/3] Publish backend (development)...
pushd "%API_DIR%"
