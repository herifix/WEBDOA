...
echo Starting IIS App Pool "%APPPOOL%" on LOCAL SERVER ...
...
if errorlevel 1 (
  echo.
  echo Gagal start App Pool "%APPPOOL%".
  echo Coba buka IIS Manager, klik kanan App Pool "%APPPOOL%", pilih Start.
  pause
  exit /b 1
)

echo.
echo App Pool "%APPPOOL%" berhasil di-start.
pause
