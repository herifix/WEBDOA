...
interface GridFullParent<T extends Row = Row> {
  columns: Column<T>[];
  rows: T[];
  maxHeight?: number;
...
const ERPGridTable = forwardRef(function ERPGridTableInner<T extends Row = Row>(
  ...
  function getVisibleJump() {
    if (heightMode === "fill" && bodyRef.current) {
      const rowHeight = 36;
      return Math.max(1, Math.floor(bodyRef.current.clientHeight / rowHeight));
    }
    return Math.max(1, Math.floor(maxHeight / 36));
  }
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLDivElement>) {
    ...
    const visibleJump = getVisibleJump();
    ...
    >
      {heightMode === "fill" ? (
        <>
          ...
          {showPagination && (
            <div className="shrink-0 flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 px-3 py-2 text-sm">
              <div className="text-slate-600">
                Total Records: <span className="font-semibold">{totalRecords}</span> | Showing{" "}
                {startRow} to {endRow} of {totalRecords} entries
              </div>
...
export default ERPGridTable as <T extends Row = Row>(
  props: GridFullParent<T> & { ref?: React.Ref<ERPGridTableRef> }
) => React.ReactElement;